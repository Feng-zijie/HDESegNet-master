...
class DenseLayer(nn.Module):
    ...
    def __init__(self, in_channels, growth_rate):
        ...
        self.depthwise = nn.Conv2d(
            ...
            kernel_size=3, 
            padding=1, 
            groups=4 * growth_rate,  # 关键：分组数=输入通道数（深度卷积）
            bias=False
        )
        self.pointwise = nn.Conv2d(
            4 * growth_rate, 
            growth_rate, 
            kernel_size=1,  # 1x1 卷积融合通道
            bias=True
        )

    def forward(self, x):
        out = self.conv1(F.relu(self.bn1(x)))
        out = self.pointwise(self.depthwise(F.relu(self.bn2(out))))
...
class CrossEncoderFusion(nn.Module):
    ...
    def forward(self, x, simplified_encoder_layers, transformer_encoder_layers, out_indices):
        outs = []
        cnn_encoder_out = x
        ...
        for i, (simplified_encoder_layer, transformer_encoder_layer) in enumerate(zip(simplified_encoder_layers, transformer_encoder_layers)):
            # CNN 分支
            cnn_encoder_out = simplified_encoder_layer(x)
