...
class MLLABlock(nn.Module):
    ...
    def forward(self, x, shortcut,fmt):
        H, W = self.input_resolution
        B, L, C = x.shape
        ...
        x = shortcut + self.drop_path(x) + fmt # 第一个残差
...
class MLLA(nn.Module):
    ...
    def forward(self, x):
        ...
        # FFN
        if self.classes:
            x = x + self.drop_path(self.mlp(self.norm2(x))) + fmt # 第二个残差
        else: 
            x = x + self.drop_path(self.mix_ffn(self.norm2(x),(H,W))) + fmt # 第二个残差
