...
def frequency_guided_mask(dwt_feat):
    # dwt_feat: [B, C, H, W]
    energy = torch.mean(torch.abs(dwt_feat), dim=1, keepdim=True)  # 高频能量
    mask = energy / (torch.max(energy) + 1e-6)
    return mask

class FrequencyGuidedFuse(nn.Module):
    def __init__(self):
        super(FrequencyGuidedFuse, self).__init__()

    def forward(self, x1, x2, dwt_feat):
        """
        x1: 局部细节分支 (CNN)
        x2: 全局语义分支 (Transformer)
        dwt_feat: 来自小波分支的高频特征
  """
        # 统一尺寸
        if dwt_feat.shape[2:] != x1.shape[2:]:
            dwt_feat = F.interpolate(dwt_feat, size=x1.shape[2:], mode='bilinear', align_corners=False)

        mask = frequency_guided_mask(dwt_feat)
        # 防止mask过强：可选平滑
        mask = torch.clamp(mask, 0.05, 0.95)
        
        # 融合
        fused = mask * x1 + (1 - mask) * x2
        return fused
...
class CrossEncoderFusion(nn.Module):
    def __init__(self, fusion_out_channels):
        ...
        self.frquency =  FrequencyGuidedFuse()
        
    def forward(self, x, cnn_encoder_layers, transformer_encoder_layers , dwt_encoder_layers , out_indices):
        outs = []
        cnn_encoder_out = x
        trans_encoder_out = x
        ...
        for i, (cnn_encoder_layer, transformer_encoder_layer, dwt_encoder_layer) in enumerate(zip(cnn_encoder_layers, transformer_encoder_layers,dwt_encoder_layers)):
            ...
            x = self.frquency(cnn_encoder_out, trans_encoder_out, dwt_encoder_out)
